agent = 1
#choose 1 for hueristic AI agent
#choose 2 for greedy AI agent
#choose 3 for random AI agent
...
class TetrisAI(object):
    def nextMove(self):
        ...
        # print("=======")
        strategy = None
        if BOARD_DATA.currentShape.shape in (Shape.shapeI, Shape.shapeZ,
                                             Shape.shapeS):
            d0Range = (0, 1)
        ...
        if BOARD_DATA.nextShape.shape in (Shape.shapeI, Shape.shapeZ,
                                          Shape.shapeS):
            d1Range = (0, 1)
            ...
            for x0 in range(-minX, BOARD_DATA.width - maxX):
                ...
                for d1 in d1Range:
                    ...
                    for x1 in range(-minX, BOARD_DATA.width - maxX):
                        score = self.calculateScore(np.copy(board), d1, x1,
                                                    dropDist)
    ...
    def calculateScore(self, step1Board, d1, x1, dropDist):
        ...
        self.dropDownByDist(step1Board, BOARD_DATA.nextShape, d1, x1,
                            dropDist[x1])
        # print(datetime.now() - t1)
        ...
        for y in range(height - 1, -1, -1):
            ...
            for x in range(width):
                if step1Board[y, x] == Shape.shapeNone:
                    hasHole = True  # checking for hole
                    holeCandidates[x] += 1
                else:
                    hasBlock = True
                    roofY[x] = height - y
            ...
            if not hasBlock:
                break
            if not hasHole and hasBlock:  # check for empty lines
                fullLines += 1
        vHoles = sum([x**.7 for x in holeConfirm])
        vBlocks = vBlocks - vHoles
        maxHeight = max(roofY) - fullLines
        sum_height = sum([y for y in roofY])
        #print("blocks", vBlocks)
        # print("holes", vHoles)
        ...
        if (agent == 1):
            score = fullLines * 20 - vHoles * 9 - maxHeight * 5
            return score
        # for Greedy AI
        score = fullLines * 10 - vHoles * 1
        if (agent == 2):
            return score
        # print(score)
        # For Random AI agent
